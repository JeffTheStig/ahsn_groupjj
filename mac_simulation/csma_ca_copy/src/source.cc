...
void SensorNodeCSMACA::handleMessage(cMessage *msg)
{
    if (msg==backoff_timer_expired)
    {
        ...
        else  // channel is busy
        {
            ...
            if (nb <= macMaxCSMABackoffs){
                ...
                cModule* c = getModuleByPath("SourceSink");
                c->par("total_retries") = ((int) c->par("total_retries")) + 1;

                // emit signals
                emit(sig_retries, (int) c->par("total_retries"));
            ...
            else
            {
                ...
                emit(sig_pkts_timeout, (int) c->par("dropped_pkts"));
            ...
            // Emit signals. Either dropped packets or retries has incremented.
            cModule* c = getModuleByPath("SourceSink");
            double tot_p = ((double) c->par("received_pkts").intValue()) + ((double) c->par("collided_pkts").intValue()) + ((double) c->par("dropped_pkts").intValue());
    ...
    else if (msg==set_channel_free[0])
    {
        if (net == 0 || net == -1) {
            ...
            if (((int) c->par("concurrent_tx_net0")) <= 1)
            {
                ...
                emit(sig_latency, (double) c->par("latency"));
            }
    ...
    else if (msg==send_message)
    {
        ...
        emit(sig_energy_cons, (double) c->par("energy"));
        // increase counters
        if (net == 0) {
